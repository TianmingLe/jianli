...
type CSSVarProps = Record<string, string>;
...
function buildGlowVars(glowColor: string, intensity: number): CSSVarProps {
  const { h, s, l } = parseHSL(glowColor);
  const base = `${h}deg ${s}% ${l}%`;
  const opacities = [100, 60, 50, 40, 30, 20, 10];
  const keys = ['', '-60', '-50', '-40', '-30', '-20', '-10'];
  const vars: CSSVarProps = {};
...
function buildGradientVars(colors: string[]): CSSVarProps {
  const vars: CSSVarProps = {};
  for (let i = 0; i < 7; i++) {
